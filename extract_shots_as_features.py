# code adopted from https://github.com/antoinekeller/tennis_shot_recognition
            ...
            if FRAME_ID - NB_IMAGES // 2 + 1 == shots.iloc[CURRENT_ROW]["FrameId"]:
                ...
                if shot_class == "forehand-volley":
                    outpath = Path(args.out).joinpath(
                        f"forehand_volley_{IDX_FOREHAND_VOLLEY:03d}.csv"
                    )
                    IDX_FOREHAND_VOLLEY += 1
                elif shot_class == "backhand-volley":
                    outpath = Path(args.out).joinpath(
                        f"backhand_volley_{IDX_BACKHAND_VOLLEY:03d}.csv"
                    )
                    IDX_BACKHAND_VOLLEY += 1
